...
#define startTimeout() (timeout_start_ms = std::chrono::system_clock::now())

// Check if timeout is enabled (set to nonzero value) and has expired
#define checkTimeoutExpired() (io_timeout > 0 && (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - timeout_start_ms)).count() > io_timeout)
...
VL53L0X::VL53L0X(std::uint8_t address = ADDRESS_DEFAULT)
  : frc::I2C::I2C(frc::I2C::kOnboard, address)
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)
{
    ...
    last_status = frc::I2C::WriteBulk(data.data(), data.size());
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
  ...
  data.push_back(reg);
  last_status = frc::I2C::WriteBulk(data.data(), data.size());
  frc::I2C::ReadOnly(data.size(), data.data());
  value = data[0];
  //Wire.beginTransmission(address);
  //Wire.write(reg);
  //last_status = Wire.endTransmission();
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
  ...
  data.resize(4);
  frc::I2C::ReadOnly(data.size(), data.data());
  value  = (uint16_t)data[0] << 24;
  value |= (uint16_t)data[1] << 16;
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const * src, uint8_t count)
{
  ...
  std::vector<std::uint8_t> data(src, src + count);
  vec.push_back(reg);
...
/*
  ...
  last_status = Wire.endTransmission();
  */
 last_status = frc::I2C::WriteBulk(vec.data(), vec.size());
}
